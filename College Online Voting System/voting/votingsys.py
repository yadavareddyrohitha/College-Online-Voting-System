...
def on_login():
    ...
    if authenticate(username, password):
        ...
        if authenticate(username, admin_password):  # Check if the username and password match admin credentials
...
welcome_bg_image = load_and_stretch_image("frame.jpeg", screen_width, screen_height)
login_bg_image = load_and_stretch_image("frame0.jpg", screen_width, screen_height)
vote_bg_image = load_and_stretch_image("frame2.jpg", screen_width, screen_height)
res_bg_image = load_and_stretch_image("frame1.jpg", screen_width, screen_height)
...
def vote():
    if voting_frozen:
        ...
        return
    candidate = selected_candidate.get()  # Retrieve the selected candidate
    if not candidate:
    ...
    if len(voted_users) == len(users):  # Check if all regular users have voted
